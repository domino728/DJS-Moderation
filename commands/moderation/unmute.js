...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!Role)
      return message.channel.send(
        `:x: There Is No Mute Role, So Member Is Not Muted Anymore!`
      );

    if (!Member.roles.cache.has(Role)) {
      return message.channel.send(`:x: Member Is Already Unmuted!`);
    }
    ...
    if (Role && Member.roles.cache.has(Role)) {
      ...
      return message.channel.send(`:x: Something Went Wrong, Try Again Later!`);
    }
